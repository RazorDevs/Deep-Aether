...
public class BlueRosewoodTreeGrower extends AbstractTreeGrower {
    ...
    protected Holder<? extends ConfiguredFeature<?, ?>> getConfiguredFeature(RandomSource random, boolean largeHive) {
        if(random.nextInt(4) == 0)
            return (Holder<? extends ConfiguredFeature<?, ?>>) DAConfiguredFeatures.BLUE_ROSEROOT_TREE_LARGE;
        else return (Holder<? extends ConfiguredFeature<?, ?>>) DAConfiguredFeatures.BLUE_ROSEROOT_TREE_SMALL;
    }
}
